...
public class CreateLabChargeFormController implements Initializable,FormController<Object> {
    ...
    @FXML
    private JFXComboBox<String> t2categF;

    @FXML
    private JFXComboBox<HospitalService> t2testF;
            ...
            FXTask task = new FXTask(){
                ...
                protected void load() {
                    try{
                        ...
                        String phys = t2physicianF.getText();
    ...
    void t2addService(ActionEvent event) {
        if(t2testF.validate() && t2patientF.validate() && t2ageF.validate() && t2genderF.validate() && t2chargedtoF.validate()){
            ...
            HospitalService selSrv = t2testF.getSelectionModel().getSelectedItem();
            
            if(selSrv != null){
                selSrv = selSrv.getModelClone();
                ...
                if(!patientfound){
                    ...
                    selSrv.addResource("patientage", t2ageF.getText());
                    selSrv.addResource("patientid", 0);
                    selSrv.addResource("patientgender", t2genderF.getSelectionModel().getSelectedItem());
                    selSrv.addResource("chargedto", t2chargedtoF.getText());
                    selSrv.addResource("physician", t2physicianF.getText());
                }
    ...
    public void loadUIFixes() {
        try{
            ...
            t2categF.getItems().setAll(categories);
            ...
            t2categF.getSelectionModel().selectedItemProperty().addListener((obs,oldVal,newVal)->{
                if(newVal != null && !newVal.isEmpty()){
                    Platform.runLater(()->{
                        List<HospitalService> t = new ArrayList();
                        for(HospitalService s:services){
                        ...
                        t2testF.getItems().setAll(t);
                        if(t.size()>0){
                            t2testF.getSelectionModel().selectFirst();
                        }
                    });
                }
            });
            
            t2testF.getSelectionModel().selectedItemProperty().addListener((obs,oldVal,newVal)->{
                if(newVal != null){
                    t2priceF.setText(NumberKit.toCurrency(newVal.getPrice()));
                }
            });
            
            if(categories.size() > 0){
                t1categF.getSelectionModel().selectFirst();
                t2categF.getSelectionModel().selectFirst();
            }
        }catch(Exception er){
            Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, null, er);
        }
