...
public class RadiologyUXController implements Initializable,UIController {
    ...
    void addConsultation(ActionEvent event) {
        try{
            ...
            lsv.setSourceHeader(new Label("Available Radiology Exams"));
            lsv.setTargetHeader(new Label("Selected Radiology Exams"));
            
            List<HospitalService> services = SQLTable.list(HospitalService.class, HospitalService.FACILITY+"='RADIOLOGY' ORDER BY "+HospitalService.CATEGORY+" ASC,"+HospitalService.DESCRIPTION+" ASC");
            lsv.setSourceItems(FXCollections.observableArrayList(services));
            final Label totalLb = new Label("Total Amount : 0.00");
            totalLb.getStyleClass().add("anton-font");
            lsv.setTargetFooter(totalLb);
            ObservableList<HospitalService> ob = lsv.getTargetItems();
            ob.addListener((javafx.collections.ListChangeListener.Change<? extends HospitalService> c) -> {
                List<HospitalService> selservices = lsv.getTargetItems();
                List<Double> sales = selservices.stream().map(itm -> itm.getPrice()).collect(Collectors.toList());
            ...
            content.getChildren().addAll(patientNameF,physicianNameF,lsv);
            

            JFXButton createBtn = new JFXButton("Create");                                    
            createBtn.getStyleClass().add("btn-danger");

            JFXDialog dialog = FXDialog.showConfirmDialog(mainStack,"Create Radiology Exam Records",content,FXDialog.DANGER,createBtn);
                ...
                if(patientNameF.validate()){
                    ...
                    protected void load() {
                        try{
                            ...
                            if(selPatient != null && lsv.getTargetItems().size() > 0){
                                ...
                                if(CHARGE.save(chargeItems)> 0){
                                    Platform.runLater(()->{       
                                        dialog.close();
                                        FXDialog.showMessageDialog(mainStack, "Success", "New Radiology records has been registered!", FXDialog.SUCCESS);
                                    });
                                    CHARGE.printChargeSlip(maskerPane, opts.get("FACILITYNAME"), "Radiology", false);
