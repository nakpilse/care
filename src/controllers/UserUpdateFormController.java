 ...
 * @author Duskmourne
 */
public class UserUpdateFormController implements Initializable,FormController<User> {
            ...
            FXTask task = new FXTask(){
                ...
                protected void load() {
                    try{
                        ...
                        Thread.sleep(200);
                        if(!record.getPassword().isEmpty() && record.update(true,User.NAME,User.PASSWORD,User.ROLE)){
                            Platform.runLater(()->{       
                                dialog.close();
                                FXDialog.showMessageDialog(stackPane, "Success", "User has been updated!", FXDialog.SUCCESS);
                            });
                            postAction();
                        }else if(record.getPassword().isEmpty() && record.update(true,User.NAME,User.ROLE)){
                            Platform.runLater(()->{       
                                dialog.close();
                                FXDialog.showMessageDialog(stackPane, "Success", "User has been updated!", FXDialog.SUCCESS);
                            });
    ...
    public boolean isFieldInputsValid() {
        return passF.validate() && cpassF.validate() && nameF.validate() && roleF.validate();
    }
        ...
        try{
            ...
            Platform.runLater(()->{
                try {
                    ...
                    Logger.getLogger(UserUpdateFormController.class.getName()).log(Level.SEVERE, null, ex);
                }
            });
    ...
    public void loadValidators() {
        try{
            ...
            FXField.addFocusValidationListener(passF,cpassF,nameF,roleF);
        }catch(Exception er){
            Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, null, er);
        }
    }
    ...
    public static FXDialogTask showDialog(User user,StackPane stackpane,MaskerPane masker, UIController ui_controller, Node... nodes) {
        if (dialog == null || !dialog.isVisible()) {
            try {
                ...
                FXMLLoader LOADER = new FXMLLoader(FormController.class.getResource("/views/UserUpdateForm.fxml"));
                user.setPassword("");
